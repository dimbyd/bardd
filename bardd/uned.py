# uned.py
'''
Llinell, Cwpled, Pennill neu Awdl

Er mwyn casglu pethau sy'n gyffredin i bob un o'r uchod,
e.e. seinegoli, dadansoddi, dangos, ayb

Dylai hyn berthyn i'r "dadansowddwr" er mwyn gallu storio
dadansoddiadau wrth ymyl y llinellau, cwpledi neu'r
penillion unigol.
'''
...
class Uned():
    ...
    def __init__(self, cynnwys=None):
        ...
        if not type(cynnwys) in (Gair, Llinell, Cwpled, Pennill, list):
            raise ValueError(
                'Mae angen `Gair`, `Llinell`, `Cwpled` neu `Pennill` fan hyn.'
            )
    ...
    def sillafau(self):
        ...
        # llinell
            ...
            sillafau = [sillaf
                        for gair in self.cynnwys.children
                        for sillaf in gair.children]

        # pennill
        elif type(self.cynnwys) is Pennill:
            sillafau = [gair
                        for llinell in self.cynnwys.children
                        for gair in llinell.children]

        # rhestr geiriau
        elif (
            type(self.cynnwys) in (list, tuple) and
            all([type(x) is Gair for x in self.cynnwys])
        ):
            sillafau = [sillaf
                        for gair in self.cynnwys
                        for sillaf in gair.children]

        # rhestr sillafau
        elif (
            type(self.cynnwys) in (list, tuple) and
            all([type(x) is Sillaf for x in self.cynnwys])
        ):
